...
ndvi_temporal_ci <- tibble(hour = seq(0, 23, length.out = 200)) %>%
  mutate(
...
ggplot(ndvi_temporal_ci, aes(x = hour)) +
  ...
  geom_line(aes(y = est)) +
  labs(x = "Hour", y = "NDVI coefficient", fill = NULL) +
  theme_classic()
...
ggplot(data = ndvi_hour_values,
  ...
  geom_contour(aes(z = ssf_linear_log_rss), colour = "black") +
...
ggsave(paste0("outputs/ndvi_harmonic_response_id", which_buffalo, ".png"),
       width = 120, height = 60, units = "mm", dpi = 600)
...
### Pull out the coefficents across time
...
#   
#   return(c(
#     linear_coef = as.numeric(linear_part),
#     quadratic_coef = as.numeric(quadratic_part)
#   ))
# 
# }
# 
...
ggplot(data = ndvi_hour_values,
  ...
  geom_raster() +
  labs(title = "harmonics(hour + hour^2)", x = "Hour", y = "NDVI", fill = "log-RSS") +
  geom_contour(aes(z = ssf_quadratic_log_rss), colour = "black") +
  scale_fill_distiller(palette = "RdBu", type = "div",
                       limits = c(-max_abs_colour, max_abs_colour)) +
  coord_cartesian(expand = FALSE) +
  theme_bw() 

ggsave(paste0("outputs/ndvi_harmonic_quadratic_response_id", which_buffalo, ".png"),
       width = 120, height = 60, units = "mm", dpi = 600)
...
ggplot(data = ndvi_hour_values,
  ...
  geom_contour(aes(z = gam_log_rss), colour = "black") +
  scale_fill_distiller(palette = "RdBu", type = "div",
                       limits = c(-max_abs_colour, max_abs_colour)) +
  coord_cartesian(expand = FALSE) +
  theme_bw() 

ggsave(paste0("outputs/ndvi_linear_gam_response_id", which_buffalo, ".png"),
       width = 120, height = 60, units = "mm", dpi = 600)
...
ssf_gam2d_temporal2 <- mgcv::gam(cbind(times, step_id_) ~ 
                                  te(hour, ndvi, bs = c("cc", "cr"), k = c(8, 10)) +
                       sl_ + log_sl + cos_ta,
                     data = buffalo_ssf_data,
                     family = cox.ph,
                     weight = case_)
...
filtered_gam_smooths <- gam_smooths %>% filter(ndvi > 0.14 & ndvi < 0.6)
...
ggsave(paste0("outputs/ndvi_tensor_gam_response_id", which_buffalo, ".png"),
       width = 120, height = 60, units = "mm", dpi = 600)
...
ggplot(data = gam_smooths %>% filter(ndvi > 0.14 & ndvi < 0.6),
       aes(x = hour, y = ndvi, z = .estimate)) +
  geom_contour_filled() +
  labs(x = "Hour", y = "NDVI", fill = "Effect") +
  coord_cartesian(expand = FALSE) +
  theme_minimal()
...
ggplot(data = filtered_gam_smooths,
       aes(hour, ndvi)) +
  geom_contour_filled(aes(z = .estimate)) +
  geom_contour(data =  filtered_gam_smooths %>% filter(.estimate > 0),
               aes(z = .estimate), binwidth = 0.25,
...
# fit an ssf gam model
ssf_gam2d_temporal <- mgcv::gam(cbind(times, step_id_) ~ 
                       te(hour, ndvi) +
                       sl_ + log_sl + cos_ta,
                     data = buffalo_ssf_data,
                     family = cox.ph,
...
summary(ssf_gam2d_temporal)

gam_smooths <- smooth_estimates(ssf_gam2d_temporal)
...
# fit an ssf gam model
ssf_gam2d_temporal3 <- mgcv::gam(cbind(times, step_id_) ~ 
                                  te(hour, ndvi, bs = c("cc", "tp"), k = c(8, 10)) +
                       sl_ + log_sl + cos_ta,
                     data = buffalo_ssf_data,
                     family = cox.ph,
                     weight = case_)

summary(ssf_gam2d_temporal3)

gratia::draw(ssf_gam2d_temporal3, rug = FALSE)

gam_smooths <- smooth_estimates(ssf_gam2d_temporal3)
filtered_gam_smooths <- gam_smooths %>% filter(ndvi > 0.14 & ndvi < 0.6)
...
# Bin into 11 equal-width intervals (you could also use quantile bins)
filtered_gam_smooths <- filtered_gam_smooths %>%
  mutate(estimate_bin = cut(.estimate, breaks = 11, include.lowest = TRUE))

# Get 11-colour diverging palette, centred at 0
palette <- brewer.pal(11, "RdBu")  # ensures blue = high, red = low
palette <- rev(brewer.pal(11, "RdBu"))  # ensures blue = low, red = high
...
ggplot(data = filtered_gam_smooths,
  ...
  #   low = brewer.pal(11, "RdBu")[11],  # blue
  #   mid = "white",
  #   high = brewer.pal(11, "RdBu")[1],  # red
  #   midpoint = 0,                      # centre at 0
  #   name = "Value"
  # ) +
...
# fit an ssf gam model
ssf_gam2d_temporal4 <- mgcv::gam(cbind(times, step_id_) ~ 
                                   s(hour, bs = "cc", k = 8) + 
                                   s(ndvi, k = 10) + 
                                   ti(hour, ndvi, bs = c("cc", "cr"), k = c(8, 10)) +
                       sl_ + log_sl + cos_ta,
                     data = buffalo_ssf_data,
                     family = cox.ph,
                     weight = case_)

summary(ssf_gam2d_temporal4)

gam_smooths <- smooth_estimates(ssf_gam2d_temporal4)

gratia::draw(ssf_gam2d_temporal4, rug = FALSE)

# main effects
draw(ssf_gam2d_temporal4, select = "s(hour)", rug = FALSE)
draw(ssf_gam2d_temporal4, select = "s(ndvi)", rug = FALSE)

# the pure interaction surface
draw(ssf_gam2d_temporal4, select = "ti(hour,ndvi)", rug = FALSE)


# Evaluate the interaction surface on a grid
E <- smooth_estimates(ssf_gam2d_temporal4, smooth = "ti(hour,ndvi)", n = 80)

ggplot(E, aes(x = hour, y = ndvi, z = .estimate)) +
  geom_contour_filled() +
  labs(x = "Hour", y = "NDVI", fill = "Effect") +
  theme_minimal()
...
# fit an ssf gam model
ssf_gam2d_temporal_hour_int <- mgcv::gam(cbind(times, step_id_) ~ 
                                   # s(hour, bs = "cc", k = 8) + 
                                   s(ndvi, k = 10) + 
                                   ti(hour, ndvi, bs = c("cc", "cr"), k = c(8, 10)) +
                       sl_ + log_sl + cos_ta,
...
summary(ssf_gam2d_temporal_hour_int)

gam_smooths <- smooth_estimates(ssf_gam2d_temporal_hour_int)

gratia::draw(ssf_gam2d_temporal_hour_int, rug = FALSE)

# main effects
# draw(ssf_gam2d_temporal_hour_int, select = "s(hour)", rug = FALSE)
draw(ssf_gam2d_temporal_hour_int, select = "s(ndvi)", rug = FALSE) +
  scale_y_continuous(limits = c(-2, 1))

# the pure interaction surface
draw(ssf_gam2d_temporal_hour_int, select = "ti(hour,ndvi)", rug = FALSE)
...
# fit an ssf gam model
ssf_gam2d_temporal5 <- mgcv::gam(cbind(times, step_id_) ~ 
                                   s(ndvi, k = 10) +
                                   s(hour, by = ndvi, bs = "cc", k = 8) + 
                       sl_ + log_sl + cos_ta,
                     data = buffalo_ssf_data,
                     family = cox.ph,
                     weight = case_)

summary(ssf_gam2d_temporal5)

gam_smooths <- smooth_estimates(ssf_gam2d_temporal5)

gratia::draw(ssf_gam2d_temporal5, rug = FALSE)
